...
namespace Sawczyn.EFDesigner.EFModel.EditingOnly
{
   ...
   public partial class GeneratedTextTransformation
   {
      ...
      public class EFCore5ModelGenerator : EFCore3ModelGenerator
      {
         ...
         {
            foreach (BidirectionalAssociation association in modelRoot.Store.ElementDirectory.AllElements
         ...
         private static bool IsGeneratedManyToManyAssociationClassLink(BidirectionalAssociation association)
         {
            if (association?.Target == null || !association.Target.IsAssociationClass || association.Target.DescribedAssociationElementId == Guid.Empty)
               return false;

            BidirectionalAssociation describedAssociation = association.Store.ElementDirectory.AllElements
                                                                             .OfType<BidirectionalAssociation>()
                                                                             .FirstOrDefault(a => a.Id == association.Target.DescribedAssociationElementId);
         ...
         private static List<string> GetAssociationClassForeignKeyPropertyNames(ModelClass associationClass, BidirectionalAssociation associationLink)
         {
            return associationClass.Attributes
                                   .Where(a => a.IsForeignKeyFor == associationLink.Id)
                                   .Select(a => a.Name)
                                   .ToList();
         }

         private static List<string> GetAssociationClassForeignKeyNames(ModelClass associationClass, BidirectionalAssociation associationLink)
         {
            List<string> foreignKeyProperties = GetAssociationClassForeignKeyPropertyNames(associationClass, associationLink);

            if (foreignKeyProperties.Any())
               return foreignKeyProperties;

            List<string> conventionForeignKeyNames = GetConventionAssociationClassForeignKeyNames(associationLink);

            // For generated many-to-many association classes, ignore FKPropertyName because
            // it may come from pluralized navigation names (e.g. ProductsId).
            if (IsGeneratedManyToManyAssociationClassLink(associationLink) && conventionForeignKeyNames.Any())
               return conventionForeignKeyNames;

            if (!string.IsNullOrWhiteSpace(associationLink.FKPropertyName))
               return associationLink.FKPropertyName
                                     .Split(',')
                                     .Select(x => x.Trim())
         ...
         private static List<string> GetConventionAssociationClassForeignKeyNames(BidirectionalAssociation associationLink)
         {
            return associationLink.Source.AllIdentityAttributes
                                 .Select(a =>
                                         {
                                            if (a.Name.StartsWith(associationLink.Source.Name, StringComparison.OrdinalIgnoreCase))
                                               return a.Name;

                                            return a.Name.Equals("Id", StringComparison.OrdinalIgnoreCase)
                                                      ? $"{associationLink.Source.Name}_Id"
                                                      : $"{associationLink.Source.Name}_{a.Name}";
                                         })
                                 .ToList();
         }

         private static string BuildForeignKeyLambdaExpression(IEnumerable<string> foreignKeyProperties)
         {
            List<string> keys = foreignKeyProperties.ToList();

            return keys.Count == 1
                      ? $"x => x.{keys[0]}"
                      : $"x => new {{ x.{string.Join(", x.", keys)} }}";
         }

         private static string BuildForeignKeyNameListExpression(IEnumerable<string> foreignKeyNames)
         {
            return string.Join(", ", foreignKeyNames.Select(n => $"\"{n}\""));
         }

         private static bool CanUseAssociationClassForManyToMany(ModelClass associationClass, BidirectionalAssociation association)
         {
            if (associationClass == null || association == null)
               return false;

            List<BidirectionalAssociation> joins = associationClass.AllNavigationProperties()
                                                                   .Select(np => np.AssociationObject)
                                                                   .OfType<BidirectionalAssociation>()
                                                                   .ToList();

            BidirectionalAssociation associationToSource = joins.FirstOrDefault(a => a.Source == association.Source);
            BidirectionalAssociation associationToTarget = joins.FirstOrDefault(a => a.Source == association.Target);

            return associationToSource != null && associationToTarget != null;
         }
         ...
         private void WriteBidirectionalAssociationWithAssociationClass(ModelClass modelClass, ModelClass associationClass, BidirectionalAssociation association)
         {
            ...
            List<string> foreignKeyPropertiesToTarget = GetAssociationClassForeignKeyPropertyNames(associationClass, associationToTarget);
            List<string> foreignKeyPropertiesToSource = GetAssociationClassForeignKeyPropertyNames(associationClass, associationToSource);
            List<string> foreignKeyNamesToTarget = GetAssociationClassForeignKeyNames(associationClass, associationToTarget);
            List<string> foreignKeyNamesToSource = GetAssociationClassForeignKeyNames(associationClass, associationToSource);
            string targetDeleteBehaviorSegment = association.TargetDeleteAction == DeleteAction.Default
                                                    ? string.Empty
                                                    : $".OnDelete({PresentationHelper.GetDeleteActionSqlString(association.TargetDeleteAction)})";
            string sourceDeleteBehaviorSegment = association.SourceDeleteAction == DeleteAction.Default
                                                    ? string.Empty
                                                    : $".OnDelete({PresentationHelper.GetDeleteActionSqlString(association.SourceDeleteAction)})";
            ...
            Output(".WithMany()");
            Output(foreignKeyPropertiesToTarget.Any()
                      ? $".HasForeignKey({BuildForeignKeyLambdaExpression(foreignKeyPropertiesToTarget)}){targetDeleteBehaviorSegment},"
                      : $".HasForeignKey({BuildForeignKeyNameListExpression(foreignKeyNamesToTarget)}){targetDeleteBehaviorSegment},");
            PopIndent();
            Output("j => j");
            PushIndent(indent);
            Output($".HasOne(x => x.{associationToSource.SourcePropertyName})");
            Output(".WithMany()");
            Output(foreignKeyPropertiesToSource.Any()
                      ? $".HasForeignKey({BuildForeignKeyLambdaExpression(foreignKeyPropertiesToSource)}){sourceDeleteBehaviorSegment},"
                      : $".HasForeignKey({BuildForeignKeyNameListExpression(foreignKeyNamesToSource)}){sourceDeleteBehaviorSegment},");
            PopIndent();
            Output("j =>");
            Output("{");
            ...
            string tableName = !string.IsNullOrWhiteSpace(association.JoinTableName)
                                  ...
                                  : useGeneratedManyToManyDefaultTableName
            ...
            if (modelClass.ModelRoot.IsEFCore6Plus && association.UseTemporalTables)
               modifiers.Add(" t.IsTemporal();");
         ...
         protected override void WriteBidirectionalNonDependentAssociations(ModelClass modelClass, List<Association> visited, List<string> foreignKeyColumns)
         {
            ...
            foreach (BidirectionalAssociation association in Association.GetLinksToTargets(modelClass)
                                                                        .OfType<BidirectionalAssociation>()
                                                                        .Where(x => x.Persistent && !x.Target.IsDependentType)
                                                                        .Where(x => !IsGeneratedManyToManyAssociationClassLink(x)))
            {
               if (visited.Contains(association))
                  continue;
               ...
               switch (association.SourceMultiplicity) // realized by property on target, but no property on target
               {
                  case Sawczyn.EFDesigner.EFModel.Multiplicity.ZeroMany:
                     ...
                     if (association.TargetMultiplicity == Sawczyn.EFDesigner.EFModel.Multiplicity.ZeroMany)
                     {
                        ...
                        if (associationClass == null || !CanUseAssociationClassForManyToMany(associationClass, association))
